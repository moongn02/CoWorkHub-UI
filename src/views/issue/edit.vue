    ...
    if (res) {
      ...
      Object.keys(issueForm).forEach(key => {
        if (res[key] !== undefined) {
          // 确保所有字段类型匹配
          if ((key === 'type' || key === 'severity' || key === 'urgency' ||
              key === 'platform' || key === 'bugCause' || key === 'sys' ||
              key === 'browser') && typeof res[key] === 'number') {
            issueForm[key] = String(res[key])
          } else {
            issueForm[key] = res[key]
          }
        }
      })
    } else {
  ...
  await issueFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        const success = await issueStore.updateIssueAction(formData)
        if (success) {
          goBack()
        }
      } catch (error) {
